/* =========================================================================
   client.js — (Highlight Playable Ends Feature)
   ========================================================================= */

// ... (All code from the top down to the Drag and Drop Logic section is the same) ...
...
function renderBoard() { boardEl.innerHTML = ''; if (boardState.length === 0) { renderPlaceholder(); } else { boardState.forEach(t => { const d = document.createElement('div'); d.className = 'tile disabled'; if (t[0] === t[1]) d.classList.add('double'); d.innerHTML = `<span>${t[0]}</span><span>${t[1]}</span>`; boardEl.appendChild(d); }); if (boardEl.children.length > 0) { boardEl.firstChild.classList.add('drop-target'); boardEl.firstChild.dataset.side = 'left'; if (boardEl.children.length > 1 || (boardState[0][0] !== boardState[0][1])) { boardEl.lastChild.classList.add('drop-target'); boardEl.lastChild.dataset.side = 'right'; } } } }
function renderPlaceholder() { const placeholder = document.createElement('div'); placeholder.className = 'tile-placeholder drop-target'; placeholder.dataset.side = 'right'; const isFirstGameRound = scores[0] === 0 && scores[1] === 0; placeholder.textContent = isFirstGameRound ? '6|6' : 'Play Tile'; boardEl.appendChild(placeholder); }
...
function handleDragStart(e, tileData, tileElement) {
    ...
    // NEW: Highlight valid plays as soon as you pick up a tile
...
function cleanupDrag() {
    ...
    // NEW: Also remove the playable-end highlights
    document.querySelectorAll('.drop-target, .playable-end-highlight').forEach(el => {
        el.classList.remove('drop-hover');
        el.classList.remove('playable-end-highlight');
    });
...
// NEW: Function to highlight valid ends on the board
function highlightPlayableEnds(tileData) {
    const boardEnds = getBoardEnds();
    if (!boardEnds) return; // No ends to play on yet

    const [p1, p2] = tileData;

    // Check left end
    if (boardEnds.leftTile && (p1 === boardEnds.leftPip || p2 === boardEnds.leftPip)) {
        boardEnds.leftTile.classList.add('playable-end-highlight');
    }

    // Check right end
    if (boardEnds.rightTile && (p1 === boardEnds.rightPip || p2 === boardEnds.rightPip)) {
        boardEnds.rightTile.classList.add('playable-end-highlight');
    }
}
...
// UPDATED: A more robust way to get the board's open pips
function getBoardEnds() {
    if (boardState.length === 0) {
        // If board is empty, the placeholder is the only target
        const placeholder = boardEl.querySelector('.tile-placeholder');
        if (placeholder) {
            // In the very first round, the 'pip' is 6. Otherwise, any pip is valid.
            const isFirstGameRound = scores[0] === 0 && scores[1] === 0;
            const requiredPip = isFirstGameRound ? 6 : null;
            return {
                leftTile: placeholder,
                leftPip: requiredPip,
                rightTile: placeholder,
                rightPip: requiredPip
            };
        }
        return null;
    }
    
    // When tiles are on the board
    return {
...
// ... (All socket.on event handlers are the same. We just add cleanupDrag() calls) ...
