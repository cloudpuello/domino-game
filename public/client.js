/* =========================================================================
   client.js — Pip-Rendered Tiles + Magnet-Zone (2025-07-08)
   ========================================================================= */
...
/* drag helper */
let dragged = { element:null, originalTile:null, tileData:null,
...
const playerInfoEl=$("playerInfo"), errorsEl=$("errors"),
      msgEl=$("messages"), pipEl=$("pipCounts");
...
function tileHTML(tile){ return halfHTML(tile[0])+'<div class="separator"></div>'+halfHTML(tile[1]); }
...
function renderBoard(){
  boardEl.innerHTML="";
  if(boardState.length===0){ renderPlaceholder(); }
  else{
...
function isTilePlayable([a,b],ends){
  const first = scores[0]===0 && scores[1]===0 && boardState.length===0;
  if(first) return a===6&&b===6;
  return ends && (a===ends.leftPip||b===ends.leftPip||a===ends.rightPip||b===ends.rightPip);
}
...
const insideExpand = (p,r)=> p.x>r.left-HIT_PADDING && p.x<r.right+HIT_PADDING &&
                             p.y>r.top -HIT_PADDING && p.y<r.bottom+HIT_PADDING;

/* highlight ends */
function markEnds([a,b]){
  const e=getBoardEnds(); if(!e) return;
...
function start(e,tile,tEl){
  ...
  const p=getPos(e); dragged.element.style.left=p.x+"px"; dragged.element.style.top=p.y+"px";
...
function move(e){
  ...
  document.querySelectorAll(".drop-target").forEach(t=>{
    ...
    if(insideExpand(p,r)){ ok=true; dragged.hoveredSide=t.dataset.side; t.classList.add("drop-hover"); }
    else t.classList.remove("drop-hover");
  });
...
/* Graphics helpers                                                           */
/* -------------------------------------------------------------------------- */
function seatPos(s){if(s===mySeat) return"self";const d=(s-mySeat+4)%4;return d===1?"right":d===2?"top":"left";}
function renderOpponents(){
  const areas={top:topEl,left:leftEl,right:rightEl};Object.values(areas).forEach(el=>el.innerHTML="");
  Object.entries(seatMap).forEach(([s,info])=>{
    s=+s;if(s===mySeat) return;const a=areas[seatPos(s)];if(!a) return;
    const n=document.createElement("div");n.textContent=`${info.name} (Seat ${s})`;a.appendChild(n);
    const hd=document.createElement("div");hd.className="player-area-hand-display";
    const cnt=handSizes[s]||0;for(let i=0;i<cnt;i++){const d=document.createElement("div");d.className="dummy-tile";hd.appendChild(d);}a.appendChild(hd);
  });
}
const renderScores=()=>{team0ScoreEl.textContent=scores[0];team1ScoreEl.textContent=scores[1];};
const renderPips=c=>{pipEl.textContent=c?Object.entries(c).map(([p,n])=>`${p}:${n}`).join(" | "):"";};
...
socket.on("updateHand",h=>{myHand=h;handSizes[mySeat]=h.length;renderHand();renderOpponents();});
...
socket.on("turnChanged",turn=>{
  ...
  const pos=seatPos(turn);(pos==="top"?topEl:pos==="left"?leftEl:pos==="right"?rightEl:handEl).classList.add("active-turn-indicator");
});

socket.on("playerPassed",({seat})=>addMsg(`Seat ${seat} passed.`));

socket.on("roundEnded",({winner,reason,points,scores:s,board})=>{
  boardState=board;scores=s;cleanupDrag();renderScores();renderBoard();
  setStatus(`Seat ${winner} wins (${reason}) +${points} pts.`);addMsg(`Seat ${winner} wins (${reason}) +${points} pts.`);
});

socket.on("gameOver",({winningTeam,scores:s})=>{
  sessionStorage.removeItem("domino_roomId");sessionStorage.removeItem("domino_mySeat");
