/* ============================================================================
 * server.js — (Final Version: Timer for Blocked Players Only)
 * ========================================================================= */
...
/* -------------------------------------------------------------------------- */
const TURN_MS = 90_000; // 90 seconds

// This function is now used to set a timeout for a player who MUST pass.
function startPassTimer(room) {
  if (room.passTimer) clearTimeout(room.passTimer);
  room.passTimer = setTimeout(() => {
    // This fires if the player failed to click "Pass" in time.
...
// This function now correctly applies the timer logic as requested.
function stepTurn(room) {
  if (room.passTimer) clearTimeout(room.passTimer); // Always clear the last timer
  ...
  if (canPlay) {
    // --- Player CAN go ---
    // NO timer is applied. Server waits for a 'playTile' event.
    room.passCount = 0;
  } else {
    // --- Player CANNOT go ---
    // Prompt them to pass AND start the 90-second timer.
    io.to(player.socketId).emit('mustPass');
    startPassTimer(room); // Start the clock for them to click "Pass"
  }
}
...
io.on('connection', socket => {
  ...
  socket.on('passTurn', ({ roomId, seat }) => {
    ...
    // The player has acted by clicking "Pass", so we must clear the timeout.
    if (room.passTimer) clearTimeout(room.passTimer);

    const canPlay = room.board.length === 0 || room.players[seat].hand.some(([x, y]) =>
      x === room.leftEnd || y === room.leftEnd || x === room.rightEnd || y === room.rightEnd
    );
    if (canPlay) {
      return io.to(socket.id).emit('errorMessage', 'You have a valid move and cannot pass.');
    }

    // Process the manual pass
    room.passCount++;
